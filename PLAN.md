...
- [x] **Dirección de mail para el aviso de cotizaciones de Eventos** (`EVENTOS_NOTIFICATION_EMAIL`) — CONFIRMADO por Juani el 17/09: usar `muelle3pde@gmail.com`. Falta que Juani lo agregue como variable de entorno en Vercel (Settings → Environment Variables, Type "Secret", sin prefijo `NEXT_PUBLIC_`). Aunque se agregue, el aviso por mail no va a mandarse hasta que también esté cargado `RESEND_API_KEY` (siguiente punto) — ambas son necesarias juntas.
...
- [x] **Dirección textual y horarios** para `/contacto` — CONFIRMADO por Juani el 17/09: "Rbla. Dr. Claudio Williman 22, 20100 Punta del Este, Departamento de Maldonado, Uruguay". Horario: invierno 10:00 a 00:00, verano extendido hasta las 2:00. Ya cargado en `src/lib/site-info.ts`.
...
## Sesión 22/09/2026 — 4 bugs confirmados por el PM sobre el deploy real, ninguno cosmético

Deploy `muelle3-8kffapvgq-...vercel.app` (`https://muelle3-web.vercel.app/`), commit `0e449e9`. El PM pidió explícitamente NO repetir el error anterior (dar el Bug 1 por arreglado sin estarlo) -- cada punto de abajo se verificó contra el deploy real con el navegador, no solo leyendo código.

**Bug 1 — IntersectionObserver de `/menu` seguía desincronizándose con scroll rápido (regresión del fix de la sesión de la tarde).** Se confirmó primero que el fix anterior SÍ está en `main` y en este deploy (commit `0e449e9`, verificado con `git log`) -- no fue que el commit no llegó, fue que el fix en sí no alcanzaba. Causa real más profunda: ese primer intento guardaba un `Map` con el ESTADO de qué secciones intersectan, pero comparaba usando `entry.boundingClientRect.top` -- un valor que es una FOTO fija tomada la última vez que esa sección cruzó un umbral, no su posición actual. Con scroll rápido, una sección que sigue "intersectando" sin volver a disparar el observer queda con un `top` viejo en el Map, y compararlo contra el de una sección que sí acaba de disparar (con `top` fresco) podía dar el ganador equivocado.

Fix real: `src/components/menu-category-nav.tsx` ahora usa el observer solo como gatillo ("algo cambió, recalculá") y en cada disparo recorre TODAS las secciones pidiendo su posición en VIVO con `getBoundingClientRect()` -- nunca un valor guardado. Elige la última sección (en orden) cuyo borde superior ya cruzó una línea de referencia fija (scrollspy clásico), lo que da el resultado correcto sin importar velocidad ni dirección del scroll.

**Bug 2 — Selector Día/Atardecer/Noche necesitaba doble toque en mobile (primer toque marcaba el botón pero no cambiaba foto/acentos).** Se probó primero contra el deploy real con clicks de mouse (viewport resizeado a mobile) y el selector respondió bien al primer click en los tres casos -- no se pudo reproducir el bug con mouse, lo cual señala un problema específico de TOUCH, no un bug de estado de React (la hipótesis original del PM). Investigación: es un bug bien documentado de Safari/iOS -- un elemento con una regla CSS `:hover` y sin manejo explícito de touch hace que iOS interprete el PRIMER toque como "simular estado hover" en vez de disparar el click, y recién el SEGUNDO toque (ya en "hover") dispara el click real. Los botones del selector tenían clases `hover:` de Tailwind (`hover:border-white/60`, `hover:bg-celeste/15`, etc.) -- exactamente el disparador de ese bug.

Fix aplicado en `src/components/mood-carousel.tsx`: mismo patrón que ya usa `gallery-photo.tsx` para el mismo motivo (`matchMedia("(hover: none)")`) -- en touch, se sacan del todo las clases `hover:` de los botones (no hay hover real en touch, no hacen falta). **Nota de honestidad:** no se pudo reproducir el bug original con el navegador disponible acá (solo simula clicks de mouse, no eventos táctiles reales de iOS/Android), así que este fix está basado en el patrón de bug más documentado y consistente con la evidencia (pasa en los 3 botones, incluido "Noche" que no tiene foto/red de por medio, descartando un problema de carga de imagen) -- pero pedimos que Juani lo confirme en su celular real después de este deploy, no se da por cerrado solo con esto.

**Bug 3 — Performance mobile mala y efectos no se ven.** Causa real encontrada y ya arreglada la noche anterior (ver sección de abajo): las 2 fotos reales del selector de moods se descargaban juntas desde el arranque del Home. Fix ya aplicado: cada foto se carga recién cuando ese mood se activa, Ken Burns solo en la capa activa. No se encontró ningún caso de animación dependiente de hover sin fallback en `historia-collage.tsx` (rediseñado esa misma noche, ya sin ningún hover) ni en el resto de `HeroExperience` (el `onMouseEnter`/`onMouseLeave` solo pausa el autoplay). El propio Bug 2 de acá arriba SÍ era un caso real de "hover sin fallback táctil" que no se había detectado hasta ahora -- ya corregido.

**Bug 4 — Overlay de navegación con alineación inconsistente.** Confirmado en el deploy real con el navegador: desde Home el overlay abre con los links alineados a la izquierda; desde Historia (y el resto de páginas con hero `text-center`) abre centrado -- reproducido y comparado con capturas de ambos casos. Causa y fix: ver sección anterior (`text-left` explícito en `nav.tsx`, ya aplicado).

Verificado con `tsc --noEmit` y `eslint` sobre los 3 archivos tocados (`menu-category-nav.tsx`, `mood-carousel.tsx`, `nav.tsx`), sin errores. Bug 4 se confirmó visualmente contra el deploy real (antes/después). Bug 1 y Bug 3 se verificaron por lectura de código + lógica (no involucran timing de touch, son reproducibles con cualquier input). Bug 2 es el único que queda pendiente de confirmación en dispositivo real -- ver nota arriba.

## Sesión 21/09/2026 (noche, cont.) — 2 bugs reales de mobile, reportados por el PM

Deploy `muelle3-8kffapvgq-...vercel.app` (commit `0e449e9`), probado en un celular real (no emulador). Dos bugs de prioridad alta -- mobile es el uso real del sitio:

**Bug 1 — Performance mobile lenta en el Home.** Causa real encontrada en `src/components/mood-carousel.tsx` (`HeroExperience`): los 3 moods del selector (Día/Atardecer/Noche) se montaban TODOS desde el primer render -- el crossfade solo los ocultaba con `opacity-0`/`opacity-100`, nunca los sacaba del DOM. Como las dos fotos reales (Día, Atardecer) son full-bleed (`fill`, `sizes="100vw"`), el navegador las descargaba a las DOS de una sola vez apenas cargaba el Home, sin importar cuál mood estuviera activo. `loading="lazy"` de `next/image` no ayudaba acá: geométricamente las dos capas ya están dentro del viewport desde el arranque (el `opacity-0` no las saca del área que mide el lazy loading), así que igual se pedían de entrada.

Fix: cada foto real ahora se monta (y por lo tanto se descarga) recién la primera vez que ese mood se vuelve activo -- "Día" entra de una porque es lo primero que se ve, "Atardecer" recién se pide cuando el autoplay llega a él (a los 6s) o el usuario lo elige a mano en el selector. Una vez montada una foto se queda montada (no se vuelve a sacar) para que el crossfade de ida y vuelta entre moods ya vistos siga andando sin recargarla. Además, la animación continua "Ken Burns" (`animate-ken-burns`, zoom lento infinito) ahora solo corre en la capa activa -- antes corría en las dos fotos reales aunque una estuviera en `opacity-0` esperando su turno, trabajo de composición de más sin ningún resultado visible.

No se encontró ningún caso real de animación dependiente de `hover` sin fallback táctil en los componentes nuevos: `HeroExperience` no depende de hover para nada (el `onMouseEnter`/`onMouseLeave` solo pausa el autoplay, no oculta nada), y el collage de `/historia` rediseñado esta misma noche (ver sección anterior) ya no tiene ningún efecto de hover -- se sacó por completo al simplificar ese componente. El único lugar del sitio con el patrón hover/touch (`gallery-photo.tsx`, `matchMedia("(hover: none)")`) sigue intacto y sin cambios.

**Bug 2 — Overlay de navegación: alineación inconsistente (izquierda desde Home, centrada desde otras páginas).** Causa real en `src/components/nav.tsx`: el overlay es `fixed`, así que su posición en pantalla no depende de dónde vive en el árbol de React -- pero `text-align` SÍ se hereda por el árbol de React/DOM, no por la posición visual final. `<SiteNav />` se renderiza adentro del `<section>` de hero de cada página, y varias de esas secciones (Historia, Menú, Eventos, Contacto) tienen `text-center` en su propia clase (para centrar el título de esa página) -- sin ningún reset explícito, ese `text-center` se colaba dentro del overlay fixed y centraba los links. El hero del Home (`page.tsx`) es el único que NO tiene `text-center` en su `<section>`, por eso desde ahí el overlay aparecía alineado a la izquierda (el valor por default) y desde cualquier otra página aparecía centrado.

Fix: se agregó `text-left` explícito a la clase del overlay en `nav.tsx` -- el layout interno siempre se manejó con flex/`items-*` (nunca dependió de `text-align` a propósito), así que este reset deja el overlay siempre igual sin importar desde qué página se abra, sin tocar nada del layout en sí.

Verificado con `tsc --noEmit` y `eslint` sobre ambos archivos, sin errores.
