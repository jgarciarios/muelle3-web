...
**Tercera pasada (noche), 4 bugs confirmados por el PM sobre el deploy `muelle3-8kffapvgq-...vercel.app` (commit `0e449e9`) — detalle completo en PLAN.md:**
- `src/components/menu-category-nav.tsx`: el fix anterior del indicador de categoría (Map de intersecciones) no alcanzaba -- seguía usando posiciones desactualizadas de secciones que no volvían a disparar el observer. Reescrito para recalcular con `getBoundingClientRect()` en vivo en cada disparo.
- `src/components/mood-carousel.tsx`: selector Día/Atardecer/Noche necesitaba doble toque en mobile -- bug conocido de Safari/iOS con elementos que tienen CSS `:hover` sin manejo de touch. Se sacan las clases `hover:` de los botones en touch (mismo patrón que `gallery-photo.tsx`). Además, cada foto del selector ahora se carga recién cuando ese mood se activa (no las dos juntas desde el arranque), y Ken Burns solo corre en la capa activa.
- `src/components/nav.tsx`: `text-left` explícito en el overlay -- confirmado en el deploy real que el `text-center` del hero de Historia (y Menú/Eventos/Contacto) se colaba por herencia de CSS y centraba el overlay, mientras que desde Home (sin `text-center`) quedaba bien.
